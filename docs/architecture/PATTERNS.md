...
// Query DTO using const pattern for sort fields
...
```typescript
// Query DTO - extends PaginatedQueryDto for common pagination fields
export class QueryTaskDto extends PaginatedQueryDto<TaskSortBy> {
  // page, limit, search, sortOrder are inherited from PaginatedQueryDto
  // Only add resource-specific filters here
...
```typescript
// Middleware adds correlation ID using async context
@Injectable()
export class CorrelationIdMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction): void {
    ...
    // Run rest of request in correlation context (async local storage)
